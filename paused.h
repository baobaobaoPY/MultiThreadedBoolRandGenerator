...
class AWML {
public:
    static inline void paused(const std::string& PROMPT, const std::string& TR_TO_SK);
    static inline void paused(const std::string& TR_TO_SK);
...
inline void AWML::paused(const std::string& PROMPT, const std::string& TR_TO_SK) {
    while (_kbhit()) {
        _getch();}
...
inline void AWML::paused(const std::string& TR_TO_SK) {
    while (_kbhit()) {
        _getch();}
...
inline void AWML::getch() {
    ...
    fflush(stdout);
    short temp = system("/bin/bash -c 'read -s -n1'");
    short Temp = system("bash -c 'while read -t0.001 -n1 -rs; do :; done'");
    fmt::print("\n");
};
...
inline std::string AWML::GetSystemLanguage() {
    ...
    while (std::getline(OpenFile, line)) {
        ...
        size_t length = (dot_pos == std::string::npos) ?
                        std::string::npos : dot_pos - start;
